...
if saveOutput
    fullFilepathExcel=strcat(folder,'/topDown-stats.xls');
    T=table(pRSb1,pIBb1,pRSb2,pIBb2,incb1RS,incb1IB,incb2RS,incb2IB);
    writetable(T,fullFilepathExcel)
    fullFilepathAnalysisResults=strcat(folder,'/topDown-analysisResults');
...
ax.YLim(1)=0;
ax.YLim(2)=100;
leg=legend('4 Hz   ','8 Hz   ','15 Hz  ','25 Hz  ','40 Hz  ','80 Hz  ','Tonic  ','Control');
leg.Box='Off';
leg.Location='northeastoutside';
leg.FontSize=18;
%leg.Position=[0.9 0.9 0.2 1];
% subplot(1,2,2)
...
if saveOutput
    fullFilepathPowerSpec=strcat(folder,'/topDown-powerSpecSummaryA');
    fullFilepathPowerSpecB=strcat(folder,'/topDown-powerSpecSummaryB');
